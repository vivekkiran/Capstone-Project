...
public class Provider extends ContentProvider {
    ...
    static {
        ...
        mContestByShowSettingsQueryBuilder.setTables(
                ContestEntry.TABLE_NAME + " JOIN " +
                        ResourceEntry.TABLE_NAME +
                        " ON " + ContestEntry.TABLE_NAME +
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (match) {
            ...
            case CONTEST_WITH_ID: {
                long id = ContestEntry.getIdFromContestUri(uri);
                retCursor = mContestByShowSettingsQueryBuilder.query(
                        db,
                        projection, //projection
                        ResourceEntry.TABLE_NAME +
            ...
            break;
            case CONTEST_LIVE: {
                long currentTime = ContestEntry.getTimeFromContestUri(uri);
                retCursor = mContestByShowSettingsQueryBuilder.query(
                        db,
                        projection, //projection
                        ResourceEntry.TABLE_NAME +
                                "." + ResourceEntry.RESOURCE_SHOW_COL +
                                " = 1 and " +
                                ContestEntry.CONTEST_START_COL + " < ? and " + ContestEntry.CONTEST_END_COL + " > ? and " +
                                ContestEntry.CONTEST_DURATION_COL + " <= " +
                                Long.toString(maxDuration),
            ...
            break;
            case CONTEST_PAST: {
                long endTime = ContestEntry.getTimeFromContestUri(uri);
                retCursor = mContestByShowSettingsQueryBuilder.query(
                        db,
                        projection, //projection
                        ResourceEntry.TABLE_NAME +
            ...
            break;
            case CONTEST_FUTURE: {
                long startTime = ContestEntry.getTimeFromContestUri(uri);
                retCursor = mContestByShowSettingsQueryBuilder.query(
                        db,
                        projection, //projection
                        ResourceEntry.TABLE_NAME +
                                "." + ResourceEntry.RESOURCE_SHOW_COL +
                                " = 1 and " +
                                ContestEntry.CONTEST_START_COL + " > ? and " +
                                ContestEntry.CONTEST_DURATION_COL + " <= " +
                                Long.toString(maxDuration),
            ...
            case RESOURCE: {
                retCursor = db.query(
                        ResourceEntry.TABLE_NAME, //table name
                        projection, //projection
                        null, //selection
                        null, //selectionArgs
                        null,
            ...
            case NOTIFICATION: {
                retCursor = db.query(
                        NotificationEntry.TABLE_NAME, //table name
                        projection, //projection
                        null, //selection
                        null, //selectionArgs
                        null,
            ...
            case NOTIFICATION_WITH_ID: {
                long id = NotificationEntry.getContestIdFromNotificationEventUri(uri);
                retCursor = db.query(
                        NotificationEntry.TABLE_NAME, //table name
                        projection, //projection
                        NotificationEntry.NOTIFICATION_CONTEST_ID_COL + " = ?", //selection
            ...
            break;
            /*case CONTEST_WITH_ID:
                return ContestEntry.CONTENT_ITEM_TYPE;
            case CONTEST_WITH_RESOURCE_ID:
                return ContestEntry.CONTENT_DIR_TYPE;
            case CONTEST_FUTURE:
                return ContestEntry.CONTENT_DIR_TYPE;
            case RESOURCE:
            ...
            case NOTIFICATION:
                return NotificationEntry.CONTENT_DIR_TYPE;
            case NOTIFICATION_WITH_ID:
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (match) {
            ...
            case NOTIFICATION: {
                ...
                if (_id > 0) {
                    returnUri = NotificationEntry.buildNotificationEventUriWithContestId(_id);
                } else {
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case CONTEST:
                deleted = db.delete(ContestEntry.TABLE_NAME, selection, selectionArgs);
                break;
            case RESOURCE:
                deleted = db.delete(ResourceEntry.TABLE_NAME, selection, selectionArgs);
                break;
            case NOTIFICATION:
                deleted = db.delete(NotificationEntry.TABLE_NAME, selection, selectionArgs);
                break;
            default:
    ...
    public int update(@NonNull Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case CONTEST: {
                rowsUpdated = db.update(ContestEntry.TABLE_NAME, values, selection,
                        selectionArgs);
            }
            break;
            case RESOURCE: {
                rowsUpdated = db.update(ResourceEntry.TABLE_NAME, values, selection,
                        selectionArgs);
            }
            break;
            case NOTIFICATION: {
                rowsUpdated = db.update(NotificationEntry.TABLE_NAME, values, selection,
                        selectionArgs);
            }
            break;
            default:
    ...
    public int bulkInsert(@NonNull Uri uri, @NonNull ContentValues[] values) {
        ...
        switch (match) {
            case CONTEST:
                tableName = ContestEntry.TABLE_NAME;
                break;
            case RESOURCE:
        ...
        if (tableName != null) {
            ...
            try {
                for (ContentValues value : values) {
                    long _id = -1;
                    if (tableName.equals(ResourceEntry.TABLE_NAME)) {
